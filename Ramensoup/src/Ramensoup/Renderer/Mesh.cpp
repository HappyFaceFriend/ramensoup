...
namespace Ramensoup
{
	Mesh::Mesh(const glm::vec3* positions, const glm::vec3* normals, const glm::vec2* texCoords, const uint32_t* indicies, size_t vertexCount, size_t indexCount)
		:m_Verticies(vertexCount), m_Indicies(indexCount), m_Name("no_name")
	{
	...
	Mesh::Mesh(const glm::vec3* positions, const glm::vec3* normals, const glm::vec2* texCoords, const uint32_t* indicies, size_t vertexCount, size_t indexCount, const zstring_view& name)
		:m_Verticies(vertexCount), m_Indicies(indexCount), m_Name(name)
	{
		for (size_t idx = 0; idx < vertexCount; idx++)
			m_Verticies[idx] = { positions[idx], normals[idx], texCoords[idx] };
		for (size_t idx = 0; idx < indexCount; idx++)
