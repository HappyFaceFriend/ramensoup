...
namespace Ramensoup
{
	...
	{
		auto vertexArray = std::shared_ptr<OpenGLVertexArray>(new OpenGLVertexArray());
		vertexArray->AddVertexBuffer(vertexBuffer);
		vertexArray->SetIndexBuffer(indexBuffer);
		m_VertexArrayMap[vertexBuffer.get()] = vertexArray;
		return vertexArray;
	}
	void OpenGLGraphicsAPI::DrawIndexed(const std::shared_ptr<VertexBuffer>& vertexBuffer, const std::shared_ptr<IndexBuffer>& indexBuffer, size_t indexCount)
	{
	...
	void OpenGLGraphicsAPI::DrawIndexed(const std::shared_ptr<OpenGLVertexArray>& vertexArray, size_t indexCount)
	{
