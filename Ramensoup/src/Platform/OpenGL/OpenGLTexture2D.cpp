...
namespace Ramensoup
{
		...
		:m_Path(path)
	{
		...
		RS_CORE_ASSERT(internalFormat && dataFormat, "Image format not supported");

		glCreateTextures(GL_TEXTURE_2D, 1, &m_RendererID);
		glTextureStorage2D(m_RendererID, 1, internalFormat, m_Width, m_Height);	//levels = mipmap size
